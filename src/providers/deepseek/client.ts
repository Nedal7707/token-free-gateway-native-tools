...
interface DeepSeekPowResponse {
	data?: { biz_data?: { challenge?: DeepSeekPowChallenge }; challenge?: DeepSeekPowChallenge };
	challenge?: DeepSeekPowChallenge;
}

interface DeepSeekChatSessionResponse {
	data?: { biz_data?: DeepSeekChatSession };
}

type BrowserEvalResult = { ok: true; data: unknown } | { ok: false; status: number; error: string };
...
export class DeepSeekWebClient extends BaseApiClient<DeepSeekWebCredentials> {
	...
	private serverHeaders(): Record<string, string> {
		return { ...this.browserHeaders(), Cookie: this.auth.cookie || "" };
	}
	...
	/**
	 * Not used directly — DeepSeek overrides `sendMessage` because
	 * its flow (PoW + streaming) cannot be expressed as a simple EvalResult.
	 */
	protected async callApi(_page: Page, _params: NormalizedSendParams): Promise<EvalResult> {
		throw new Error("DeepSeek uses custom sendMessage; callApi is not reachable.");
	}
		...
		const evalResult = (await page.evaluate(
			async ({
				...
				const res = await fetch("https://chat.deepseek.com/api/v0/chat/create_pow_challenge", {
					method: "POST",
					credentials: "include",
					headers: { ...headerRecord, "Content-Type": "application/json" },
					body: JSON.stringify({ target_path: tp }),
				});
				if (!res.ok) {
	...
	async solvePow(challenge: DeepSeekPowChallenge): Promise<number> {
		...
		if (algorithm === "DeepSeekHashV1") {
			...
			add_to_stack(16);
			if (status === 0) throw new Error("DeepSeekHashV1 failed to find solution");
			console.log(
				`[DeepSeekWebClient] DeepSeekHashV1 solved in ${Date.now() - start}ms, answer: ${answer}`,
			);
			return answer;
		}
	...
	async uploadFile(fileData: Buffer, fileName: string) {
		...
		const data = (await res.json()) as { data?: { biz_data?: { id?: string } } };
		const fileId = data.data?.biz_data?.id;
		if (!fileId) throw new Error("File upload response missing file id");
		let attempts = 0;
		while (attempts < 30) {
			...
			if (pollRes.ok) {
				const pollData = (await pollRes.json()) as {
					data?: { biz_data?: { files?: Array<{ status?: string }> } };
				};
				const file = pollData.data?.biz_data?.files?.[0];
				if (file?.status === "SUCCESS") return fileId;
				if (file?.status === "FAILED") throw new Error(`File parsing failed for ${fileId}`);
			}
